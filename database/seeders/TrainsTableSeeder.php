...
class TrainsTableSeeder extends Seeder
{
    ...
    public function run(Faker $faker): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $newTrain->codice_treno = rand(100, 200);
            $newTrain->azienda = $faker->randomElement(['Trenitalia', 'Italo', 'Trenord', 'Frecciarossa']);
            $newTrain->stazione_di_partenza = $faker->city;
            $newTrain->stazione_di_arrivo = $faker->city;
            $newTrain->orario_di_partenza = $faker->dateTimeBetween('now', '+1 week');
            $newTrain->orario_di_arrivo = $faker->dateTimeBetween('+2 hours', '+1 week');
            $newTrain->totale_carrozze = rand(5, 15);
